...
class RecapAnnonceDemandeListPort extends Component {
    constructor(props) {
        super(props);
        this.state = {
            price: 30,
    ...
    componentDidMount() {
        axios.post('api/profil/liste/demande/bagagiste', {
            token: this.props.location.state.client.token,
            bagagiste_id: this.props.location.state.myReservation.id
        }).then(res => {
            if (res.data.status)
                this.setState({myReservation: res.data.bagagistes}, () => {
    ...
    refuser() {
        axios.post('api/baggagite/query/refus/demande', {
            "token": this.props.location.state.client.token,
            "raisonRefus": this.state.raisonRefus,
            "client_id": this.state.clientDetails.id,
            "order_id": this.state.clientDetails.id_bagagiste_query,
    ...
    soumettre() {
        axios.post('api/baggagite/query/valide/demande', {
            "token": this.props.location.state.client.token,
            "client_id": this.state.clientDetails.id,
            "order_id": this.state.clientDetails.id_bagagiste_query,
    ...
    render() {
        ...
        const {loading, myReservation, price, previewVisible, previewImage, fileList, confierDepot, listeContenu} = this.state;
        ...
        if (this.state.redirectChat) {
            ...
            return <Redirect to={{pathname: '/annonces', state: {status: 1,statusName:this.state.status==1&&t("page_annonce.ann_enCours")}}}/>;
        else return (
            loading ?
                ...
                <div>
                    <section className={'depotAnnonce recapDemandeListe container text-center py-5'}>

                        <p className={'text-left'}>{t('circuit_depot_annonce.recapAnn')}</p>
                        ...
                        <p className={'text-left'}>{t('recap.ListeDemande')}</p>
                        <div className={"row "}>
                            {myReservation.clients.map(client =>
                                <div className={"col-md-4 "}>
                                    <div className={"recapBorderedBlock p-0 mt-4"} style={{borderRadius: 24}}>
                                        <div className={"row m-0"}>
                                            ...
                                            <div
                                                className={"col-lg-4 d-flex justify-content-center flex-column text-center p-0"}>
                                                ...
                                                {client.isValid === 0 ? <div
                                                        className={"mb-0 text-gris pointer"}
                                                        onClick={() => {
                                                            this.setState({
                                                                clientDetails: client,
                                                                price: client.price_porteur,
                                                                priceNet: client.priceNet,
                                                            }, () => {
                                                                this.setState({isDetails: !this.state.isDetails,})
                                                            })
                                                        }}>{this.state.isDetails ?
                                                    <FontAwesomeIcon icon={faChevronUp}/> : t('btns.details')}</div> :
                                                    <div onClick={() => {
                                                        this.setState({
                                                            clientDetails: client,
                                                            price: client.price_porteur,
                                                         ...
                                                         className={client.isValid == 1 ? " mb-0 text-success fs-small pointer" : " mb-0 text-danger fs-small pointer "}
                                                    >{client.isValid == 1 ? client.isPaied == 1 ?
                                                        t('page_annonce.Paye') :
                                                        t('page_annonce.demandeValider') : t('page_annonce.demandeRefuser')}</div>}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {this.state.isDetails && this.state.clientDetails ?
                            <>
                                <div className={"recapBorderedBlock text-left px-5 py-4 mt-4"}>
                                    ...
                                    <div className={"recapBorderedBlock text-left px-5 py-4 my-4"}>
                                        <div className={'row'}>
                                            <div className={'col-md-12 '}>
                                                {this.state.clientDetails.contenuTransporter.map(cont =>
                                                    <label style={{
                                                ...
                                                {!this.state.clientDetails.contenuTransporter.length &&
                                                <p>le propriétaire n'indique pas sa liste des contenus.</p>}
                                    ...
                                    <div className={'row text-left'}>
                                        <div className={'col-md-4 text-left'}>
                                            <div className={'row text-left'}>
                                                ...
                                                <div className={'col-4 text-left px-1'}>
                                                    <input type={"text"} name={"dimensionsLong"}
                                                           value={this.state.clientDetails.dimensionsLong + ' cm'}
                                                           readOnly/>

                                                </div>
                                                <div className={'col-4 text-left px-1'}>
                                                    <input type={"text"} name={"dimensionsLarg"}
                                                           value={this.state.clientDetails.dimensionsLarg + ' cm'}
                                                           readOnly/>

                                                </div>
                                                <div className={'col-4 text-left px-1'}>
                                                    <input type={"text"} name={"dimensionsH"}
                                                           value={this.state.clientDetails.dimensionsH + ' cm'}
                                                           readOnly/>
                                        ...
                                        <div className={'col-md-4 text-left'}>
                                            <div className={'row text-left'}>
                                                ...
                                                <div className={'col-md-4  text-left'}>
                                                    <input type={"text"} name={"dimensionsKg"}
                                                           value={this.state.clientDetails.dimensionsKg + 'kg'}
                                                           readOnly/>
                                    ...
                                    <div className={'row text-left'}>
                                        <div className={'col-md-6 text-left'}>
                                            <div className={'row text-left'}>
                                                <div className={'col-12 text-left'}>
                                                    <label>{t('page_home.ville_depart_propose')} <LazyLoadImage
                                                        src={"/images/" + this.state.clientDetails.type_adresse_depart.toLowerCase().replace(' ', '') + ".png"}
                                                        alt={this.state.clientDetails.type_adresse_depart} height={30}/>
                                                    </label>
                                                    <input type={"text"} name={"ville_depart"}
                                                           value={this.state.clientDetails.ville_depart}
                                                           readOnly/>
                                                    <input type={"text"} name={"adresse_point_depart"}
                                                           value={this.state.clientDetails.adresse_point_depart}
                                                           readOnly/>
                                        ...
                                        <div className={'col-md-6 text-left'}>
                                            <div className={'row text-left'}>
                                                <div className={'col-12 text-left'}>
                                                    <label>{t('page_home.ville_arrivee_propose')} <LazyLoadImage
                                                    ...
                                                    <input type={"text"} name={"ville_arrivee"}
                                                           value={this.state.clientDetails.ville_arrivee}
                                                           readOnly/>

                                                    <input type={"text"} name={"adresse_point_arrivee"}
                                                           value={this.state.clientDetails.adresse_point_arrivee}
                                                           readOnly/>
                                                </div>
                                                <div className={'col-md-6 text-left'}>
                                                    <label>{t('page_home.date')}</label>
                                    ...
                                    <div className={"recapBorderedBlock text-left px-5 py-4 my-4"}>
                                        <div className="clearfix text-md-left text-center">
                                            <Upload
                                                ...
                                                fileList={this.state.clientDetails.gallery}
                                ...
                                <div className={"row justify-content-end gap-3 mt-3"}>
                                    <div className={"col-md-4"}>
                                        <div
                                            ...
                                            {this.state.clientDetails.setting_price.map(setting =>
                                                <div className={'d-flex justify-content-between py-2'}>
                                        ...
                                        {this.state.isDetails ? <div
                                            className={"btnCustomColor mb-3 d-flex flex-row align-items-center justify-content-between w-100 text-blue border-blue bg-transparent py-1"}>
                                        ...
                                        {this.state.clientDetails.isValid === 0 &&
                                        <button onClick={() => this.soumettre()}
                                                className={"btnBlue w-100"}>{t('PreReserver')}
                                        </button>}
                                    </div>
                                    {this.state.clientDetails.isValid === 0 && <div className={"col-md-4"}>
...
export default withTranslation()(RecapAnnonceDemandeListPort);