...
final class PolitiqueConfidentialiteAdmin extends AbstractAdmin
{
     public function __construct(
        ...
        PolitiqueConfidentialiteRepository $politiqueConfidentialiteRepository
    ) {
        parent::__construct($code, $class, $baseControllerName);

        $this->politiqueConfidentialiteRepository = $politiqueConfidentialiteRepository;
        if(count($politiqueConfidentialiteRepository->findAll())==0)
        {
            $pc=new PolitiqueConfidentialite();
            $pc->settext('default value');
            $this->politiqueConfidentialiteRepository->save($pc,true);
        }
